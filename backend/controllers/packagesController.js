...
module.exports = {
    ...
    addPackage: async function(req, res) {
        ...
        let packageDoc = new Package({
            ...
            driverId: new ObjectId(aPackage.driver_id)
        });
        await packageDoc.save();
        await incrementCRUDCounter("create");

        let allocatedDriver = await Driver.findOne({_id: new ObjectId(aPackage.driver_id)});
    ...
    updatePackageById: async function(req, res) {
        try {
            let packageId = new ObjectId(req.body.package_id);
